...
public class A11DFS {
 ...
 public static void main(String[] args) throws FileNotFoundException {
  ...
  inputGraph.close();
  UnweightedGraphWithNonrecursiveDFS<Integer> graph = new UnweightedGraphWithNonrecursiveDFS<>(edges, vertices);
  graph.printEdges();
 ...
 static abstract class AbstractGraph<V> implements Graph<V> {
  ...
  public Tree dfs(int v) {
   ...
   boolean[] VisitTOF = new boolean[vertices.size()];

   // Recursively search
   dfs(v, parent, searchOrder, VisitTOF);
  ...
  private void dfs(int v, int[] parent, List<Integer> searchOrder,
   ...
   searchOrder.add(v);
   VisitTOF[v] = true; // Vertex v visited

   for (int i : neighbors.get(v)) {
    if (!VisitTOF[i]) {
     parent[i] = v; // The parent of vertex i is v
     dfs(i, parent, searchOrder, VisitTOF); // Recursive search
    }
   }
  }
  ...
  public Tree bfs(int v) {
   ...
   VisitTOF[v] = true; // Mark it visited
   ...
   while (!queue.isEmpty()) {
    ...
    for (int w : neighbors.get(u)) {
     if (!VisitTOF[w]) {
      ...
      VisitTOF[w] = true; // Mark it visited
     }
    }
   }
 ...
 static class UnweightedGraphWithNonrecursiveDFS<V> extends UnweightedGraph<V>
 {
  //Create an empty graph 
  public UnweightedGraphWithNonrecursiveDFS() {
  }

  //Edges and vertices stored 
  public UnweightedGraphWithNonrecursiveDFS(int[][] edges, V[] vertices) {
   super(edges, vertices);
  }

  //Create a graph from edges and vertices stored
  public UnweightedGraphWithNonrecursiveDFS(List<Edge> edges, List<V> vertices) {
   super(edges, vertices);
  }

  //Create a graph for integer vertices and edges
  ...
  public Tree dfs(int v) {
   ...
   return new Tree(v, parent, traversal);
  }
